...
DEBUG_CONFIG="scripts/package/truenas/extra.config"
#DEBUG_KERNEL=1
...
make_bootstrapdir() {
	...
	mkdir -p ${TMPFS}
	if [ $HAS_LOW_RAM -eq 0 ] || [ CACHENAME = "package" ] ; then
		echo "doing the thing"
		mount -t tmpfs -o size=8G tmpfs ${TMPFS}
	else
		echo "skipping the thing"
		#mk_fake_tmpfs
	fi
...
build_normal_dpkg() {
	...
	# Copy and record each built packages for cleanup later
	chroot ${DPKG_OVERLAY} apt -y install /${pkgdir}/linux-headers-truenas-amd64
	chroot ${DPKG_OVERLAY} apt -y install /${pkgdir}/linux-image-truenas-amd64
	for pkg in $(ls ${DPKG_OVERLAY}${pkgdir}/*.deb ${DPKG_OVERLAY}${pkgdir}/*.udeb 2>/dev/null)
	do
		basepkg=$(basename $pkg)
		mv ${DPKG_OVERLAY}${pkgdir}/$basepkg ${PKG_DIR}/ || exit_err "Failed mv of $basepkg"
...
custom_rootfs_setup() {
	...
	# so initrd we make now will only be used on the first boot after FreemBSD upgrade.
